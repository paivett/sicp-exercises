...
(define (install-sparse-term-list)
    ...
    (put 'the-empty-termlist 'sparse-list (tag (the-empty-sparse-termlist)))
...
(define (install-dense-term-list)
    ...
    (put 'the-empty-termlist 'dense-list (tag (the-empty-dense-termlist)))
    (put 'first-term '(dense-list) first-dense-term)
...
(define (sparse-empty-termlist) (get 'the-empty-termlist 'sparse-list ))
(define (dense-empty-termlist) (get 'the-empty-termlist 'dense-list ))

; The adjoin term is defined like this instead of using the apply-generic because we must look
; for an implementation only by the first argument, the term does not have a type tag, is a raw pair
...
(define p1terms
  (adjoin-term (make-term 3 4) (adjoin-term (make-term 1 2) (dense-empty-termlist))))
(define p1 (make-polynomial 'x p1terms))

(define p2terms
  (adjoin-term (make-term 1 6) (adjoin-term (make-term 0 0) (sparse-empty-termlist))))
(define p2 (make-polynomial 'x p2terms))

(define p3 (make-polynomial 'x (dense-empty-termlist)))
